...
import async.B_SendData;
import async.T_async;

public class SendToServer implements AsyncResponse{
	...
	ArrayList<B_SendData> mArrayList = null;
	public static volatile int count =0;
	
	
	public SendToServer(DbHelper db, Context ctx, Boolean isFront,
			String phase, String currJob, String procceNphase, Class cls) {
		this.clscontext = ctx;
		...
		this.start = cls;
		settings = ctx
				.getSharedPreferences(ctx.getString(R.string.app_name), 0);
	...
	String inspect_data() {
		...
		if (phase == "INSPECTION_BASICS")
			cursor = SelectDb.getPhaseByData(db, phase, currJob, true, true,
					true);
		else
			cursor = SelectDb.getPhaseByData(db, phase, currJob, true, true,
					false);
		...
		if (cursor != null) {
			for (int i = 0; i < cursor.getColumnCount(); i++) {
				try {
					jsonObject.accumulate(cursor.getColumnName(i),
							cursor.getString(i));
				} catch (Exception e) {
	...
	String powutil_data() {
		...
		Cursor cursor = SelectDb.getPhaseByData(db, phase, currJob, true, true,
				false);
		try {
			if (cursor != null) {
	...
	public MultipartEntity build_multipart() {
							...
							if (cursor.getColumnName(i).contains("img")
								...
								key = phase
										...
										+ cursor.getString(cursor
												.getColumnIndex("inspect_type"))
										+ "_" + cursor.getColumnName(i);
								File attach = new File(cursor.getString(i));
								if (attach.exists())
									multipartEntity.addPart(key, new FileBody(
											attach));
							} else if (cursor.getColumnName(i).contains("imp")
									&& !cursor.getColumnName(i)
											.contains("name")
									&& !cursor.getString(i).isEmpty()) {
								key = phase
		...
		try {
			multipartEntity.addPart("data",
					new StringBody(jsonArray.toString()));
	...
	public String frontEndSend() {
		InputMethodManager inputManager = (InputMethodManager) ((Activity) clscontext)
				.getSystemService(Context.INPUT_METHOD_SERVICE);
		inputManager.hideSoftInputFromWindow(((View) ((Activity) clscontext)
				.findViewById(R.id.savephase)).getWindowToken(),
				InputMethodManager.HIDE_NOT_ALWAYS);
		((Button) ((Activity) clscontext).findViewById(R.id.synchtoserver))
				.setVisibility(View.VISIBLE);
		((Button) ((Activity) clscontext).findViewById(R.id.synchtoserver))
				.setText("Sync to server");
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				if (Boolean.parseBoolean(jsonObject.getString("status"))) {
					long suc = -1;
					if (jsonObject.getString("phase").equals(
							"INSPECTION_BASICS")) {
						// suc = UpdateDb.updateIdOnServerCreate(db, clscontext,
						// currJob,
						// Integer.parseInt(jsonObject.getString("id")));
						editor.putInt("jobInEdit",
								Integer.parseInt(jsonObject.getString("id")));
						editor.putInt("isOldJob", 1);
						editor.commit();
						try {
							((Button) ((Activity) clscontext)
					...
					Cursor status = SelectDb.getstatusNmodeByPhase(db,
							jsonObject.getString("phase"),
							jsonObject.getString("id"));
					suc = UpdateDb.updatePhaseStatus(db, clscontext,
							jsonObject.getString("id"),
							jsonObject.getString("phase"), "saved", "read");
					if (suc > 0) {
						Toast.makeText(clscontext, "Data Saved.",
	...
	public MultipartEntity build_alldata_multipart() {
		...
		MultipartEntity multipartEntity = new MultipartEntity(
				HttpMultipartMode.BROWSER_COMPATIBLE);
		Cursor all_phases_stat = SelectDb
				.getstatusNmodeByPhase(db, "", currJob);

		JSONObject toSendObj = new JSONObject();
		if (all_phases_stat != null) {
			// TODO My Edit
			mArrayList = new ArrayList<B_SendData>();

			for (int k = 0; k < all_phases_stat.getCount(); k++) {
				JSONArray jsonArray = new JSONArray();
				phase = all_phases_stat.getString(all_phases_stat
						.getColumnIndex("phase"));
				if (k == 0)
					isold = all_phases_stat.getInt(all_phases_stat
							.getColumnIndex("isold"));
				if (all_phases_stat.getString(
						all_phases_stat.getColumnIndex("save_status")).equals(
						"tempsaved")) {
					// UpdateDb.updatePhaseStatus(db, this, editJobId,
					// "INSPECTION_BASICS", "tempsaved", "modified");
					Cursor cursor = SelectDb.getPhaseByData(db, phase, currJob,
							true, true, false);

					if (cursor != null && cursor.getCount() != 0) {
						UpdateDb.updatePhaseStatus(db, clscontext, currJob,
								phase, "sending", "write");
						for (int j = 0; j < cursor.getCount(); j++) {
							JSONObject eachObj = new JSONObject();
							for (int i = 0; i < cursor.getColumnCount(); i++) {
								...
								try {
									if (cursor.getColumnName(i).contains("img")
										...
										mArrayList.add(new B_SendData(phase,
												currJob, cursor
														.getColumnName(i),
												cursor.getColumnName(1), cursor
														.getString(1), cursor
														.getString(i)));
									...
									} else if (cursor.getColumnName(i)
											.contains("imp")
											&& !cursor.getColumnName(i)
													.contains("name")
											&& !cursor.getString(i).isEmpty()) {
			...
			try {
				...
				multipartEntity.addPart("data",
						new StringBody(toSendObj.toString()));
	...
	public void backEndSend() {
		...
		if (cr != null && cr.getCount() != 0)
			for (int i = 0; i < cr.getCount(); i++) {
				...
				new HttpAsyncSaveAllTask().execute(IrishreloAccess.WEBSERVICE
						+ "save_all_data");
				cr.moveToNext();
			}
	}
			...
			try {
				...
				if (Boolean.parseBoolean(jsonObject.getString("status"))) {
					...
					if (jsonObject.getString("operation").equals("INSERT"))
						UpdateDb.updateIdOnServerCreate(db, clscontext,
								jsonObject.getString("id"),
								jsonObject.getString("id"));
					if (currJob.equals(settings.getString("jobInEdit", "0"))) {
						editor.putString("jobInEdit",
								jsonObject.getString("id"));
						editor.putInt("isOldJob", 1);
						editor.commit();
					}
					...
					while (it.hasNext()) {
						...
						if (Boolean.parseBoolean(updated.get(key) + ""))
							suc = UpdateDb.updatePhaseStatus(db, clscontext,
									jsonObject.getString("id"), key, "saved",
									"read");
						else if (!Boolean.parseBoolean(updated.get(key) + "")) {
							suc = UpdateDb.updatePhaseStatus(db, clscontext,
									jsonObject.getString("id"), key,
									"tempsaved", "modified");
						}
					}
					if (suc > 0) {
					...
					try {
						IrishreloAccess.editbtnonsynch.setTag(jsonObject
								.getString("id") + "saved");
						((Button) ((Activity) clscontext)
								.findViewById(R.id.sendemail))
								.setVisibility(View.VISIBLE);
					...
					try {
						// if(isFront &&
						// start.getName().contains("ListReports"))
						{
							IrishreloAccess.synchbtn
									.setVisibility(View.VISIBLE);
							IrishreloAccess.editbtnonsynch.setClickable(true);
						}
						UpdateDb.updateAllStatusOnNoResp(db, clscontext,
								currJob);
						Toast.makeText(clscontext,
								"Data could not synchronized, try later!!.",
								Toast.LENGTH_SHORT).show();
					} catch (Exception e) {
	...
	/**
	 * first check if there is data to upload
	 * then initialize the asynctask
	 * then call the httpService.
	 */
	public void doImageUploadIng()
	{
		
		if(mArrayList != null && count < mArrayList.size())
		{
			task = new T_async();
			task.callback = this;
			task.execute(new Object[]{ "http://192.168.1.7/Wordpress/irsApp/irswebservice/imageUpload", mArrayList.get(count)});
		}
		else 
			count =0;
	}
	...
	public void onResponse(String result) {
		Log.v("Log for Upload Image", result);
		count++;
		doImageUploadIng();
	}
}